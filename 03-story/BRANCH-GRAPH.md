...
[Prologue] → [Ch.1 공통] → [Ch.2 공통] → [Ch.3 공통] → [Ch.4 공통] → [Ch.5 평가]
                                                                          ...
                                                                          └─ 모든 호감도 <65 → [혼자 여름방학] → END_SOLO_SUMMER (16번째 엔딩, 진입선 30→65 상향)
```
...
| `END_SOLO_SUMMER` | **혼자 여름방학** | NORMAL | **모든 호감도 <65 (단독 엔딩, 16번째)** |
...
```json
{
  "endings": [
    ...
    },
    {
      ...
      "bgm": "bgm_daily",
      "title": "혼자 여름방학",
      "note": "2026-04-28 추가 확정 (16번째 엔딩). 모든 히로인 호감도 <65일 때 발동 (진입선 30→65 상향). STORY-BIBLE §10."
    }
    // ... 나머지 12개
  ]
}
...
function determineMainHeroine(state: GameState): MainBranch {
  ...
  // 모든 호감도 <65 → 단독 엔딩 (16번째, 2026-04-28 확정 / 진입선 30→65 상향)
  if (max < 65) {
    return "SOLO";
  }
...
5. **END_SOLO_SUMMER 도달성**: 모든 호감도 <65로 끝나는 경로 ≥1개
